...
for ii = 1:length(v_range)
    ...
    rpm_calc = round(v * primary * gear_ratio * finaldrive * (60/(2*pi)) / r_tire / 100) * 100;
...
for i = 1:numel(v_range)
    ...
    while abs(Ay_in - Ay_last) >= a_tol
        ...
        % Calculate maximum lateral forces for front wheels
        SA = 0:SA_res:SA_max;
        
        % Left front wheel
        Fy_1_array = zeros(size(SA));
        for sa_i = 1:length(SA)
            Fy_1_array(sa_i) = pacejka_tire_model(tire_coeff_lat, [Fz_1, SA(sa_i), mulat]);
        end
        [Fy_1, I1] = max(Fy_1_array);
        alpha_1 = SA(I1);
        
        % Right front wheel
        Fy_2_array = zeros(size(SA));
        for sa_i = 1:length(SA)
            Fy_2_array(sa_i) = pacejka_tire_model(tire_coeff_lat, [Fz_2, SA(sa_i), mulat]);
        end
        [Fy_2, I2] = max(Fy_2_array);
        alpha_2 = SA(I2);
        
        % Rear axle with slip angle difference
        SA_range_3 = SA_range + SA_delta_r;
        Fy_range_3 = zeros(size(SA_range));
        for sa_i = 1:length(SA_range)
            if SA_range_3(sa_i) >= 0 && SA_range_3(sa_i) <= SA_max
                Fy_range_3(sa_i) = pacejka_tire_model(tire_coeff_lat, [Fz_3, SA_range_3(sa_i), mulat]);
            end
        end
        
        Fy_range_4 = zeros(size(SA_range));
        for sa_i = 1:length(SA_range)
            Fy_range_4(sa_i) = pacejka_tire_model(tire_coeff_lat, [Fz_4, SA_range(sa_i), mulat]);
        end
    ...
    while abs(Ax_brake_in - Ax_last) > a_tol
        ...
        SR_brake = 0:-SR_res:-SR_max;
        
        Fx_1_array = zeros(size(SR_brake));
        for sr_i = 1:length(SR_brake)
            Fx_1_array(sr_i) = pacejka_tire_model(tire_coeff_lon, [Fz_1_brake, SR_brake(sr_i), mulon]);
        end
        Fx_1 = -min(Fx_1_array);
        
        Fx_2_array = zeros(size(SR_brake));
        for sr_i = 1:length(SR_brake)
            Fx_2_array(sr_i) = pacejka_tire_model(tire_coeff_lon, [Fz_2_brake, SR_brake(sr_i), mulon]);
        end
        Fx_2 = -min(Fx_2_array);
        
        Fx_3_array = zeros(size(SR_brake));
        for sr_i = 1:length(SR_brake)
            Fx_3_array(sr_i) = pacejka_tire_model(tire_coeff_lon, [Fz_3_brake, SR_brake(sr_i), mulon]);
        end
        Fx_3 = -min(Fx_3_array);
        
        Fx_4_array = zeros(size(SR_brake));
        for sr_i = 1:length(SR_brake)
            Fx_4_array(sr_i) = pacejka_tire_model(tire_coeff_lon, [Fz_4_brake, SR_brake(sr_i), mulon]);
        end
        Fx_4 = -min(Fx_4_array);
...
for i = 1:numel(d)
    if r(i) > 0  % Corner segment
        ...
        while corner_conv ~= 1
            % Corner simulation inline
            v_corner = sqrt(Ay(v_check) * r(i));
            t_corner = d(i) / v_corner;
            t_corner = round(t_corner, roundoff);
            
            time_c = 0:dt:t_corner;
            v_c = ones(1, numel(time_c)) * v_corner;
            throttle_c = 0.3 * ones(1, numel(time_c));
        ...
        % Simplified straight simulation for now
        % Use constant velocity based on adjacent corners
        v_straight = v_max * 0.8;  % Simplified assumption
        t_straight = d(i) / v_straight;
        
        time_s = 0:dt:t_straight;
        v_s = ones(1, numel(time_s)) * v_straight;
        throttle_s = ones(1, numel(time_s));
...
for q = 1:limits(1)
    segment_times = time_out(q, time_out(q,:) > 0);
    segment_velocities = v_out(q, 1:length(segment_times));
    segment_throttles = throttle_out(q, 1:length(segment_times));
    
    if ~isempty(segment_times)
        time_vec = [time_vec, segment_times + ref];
        v_vec = [v_vec, segment_velocities];
        throttle_vec = [throttle_vec, segment_throttles];
        ref = ref + max(segment_times);
    end
end
...
while corner_conv ~= 1
    v_skid_calc = sqrt(Ay(v_check) * r_skid);
    t_skid = r_skid * 2 * pi / v_skid_calc;
...
function Y = pacejka_tire_model(beta, in)
    % Inputs: beta = coefficients, in = [Fz, X, mu]
    ...
    inner_term = B*x - E*(B*x - atand(B*x));
    y = D * sind(C * atand(inner_term));
    Y = y + SV;
